...
function OpportunityDrilldown({ opportunity, onClose, onOverride, onRescore }: { opportunity: Opportunity; onClose: () => void; onOverride: (value: number) => void; onRescore: (opportunity: Opportunity, ratings: Record<string, number>, notes: string) => void }) {
  ...
  const fieldValue = (label: string) => opportunity.dealData?.[`qualification:${label}`] || (label === "Engagement / activity" ? `${opportunity.source} lead. Next step: ${opportunity.nextStep}.` : "No qualification field value has been entered.");
  ...
  return <tr className="drilldown-row"><td colSpan={10}><div className="drilldown">
    ...
    <section className="override"><button className="btn secondary" onClick={() => setEditing(!editing)}>Override overall score</button>{editing && <div className="override-form"><label>New score<input type="number" min="0" max="100" value={score} onChange={e => setScore(Number(e.target.value))}/></label><label>Reason <textarea value={reason} onChange={e => setReason(e.target.value)} placeholder="Required to preserve the scoring audit trail" /></label><button disabled={!reason.trim()} className="btn primary" onClick={() => { onOverride(score); setEditing(false); }}>Save override</button></div>}</section>
  </div></td></tr>;
}
...
function DealWorkspace({ onClose, onSave }: { onClose: () => void; onSave: (deal: Omit<Opportunity, "score" | "tier" | "changed" | "confidence">) => void }) {
  ...
  const submit = () => {
    ...
    onSave({ ...opportunity, dealData: { ...data, qualificationNotes: data.qualificationNotes || "", ...Object.fromEntries(Object.entries(ratings).map(([key, value]) => [`rating:${key}`, String(value)])) } });
  };
