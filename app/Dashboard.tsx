...
function sampleQualification(account: string, partner: string, base: number): DealData {
  const rating = (offset = 0) => String(Math.max(1, Math.min(5, base + offset)));
  ...
  return {
    ...
    "rating:Ideal customer fit": rating(),
    "qualification:Pain and urgency": strong ? "The current manual compliance process is delaying audits and consuming significant staff time; leadership has asked for a remedy this quarter." : developing ? "The customer acknowledges audit-preparation delays, but the operational impact and urgency have not been quantified." : "General interest has been expressed, but no measurable business pain or urgent consequence has been documented.",
    "rating:Pain and urgency": rating(strong ? 0 : -1),
    "qualification:Buying authority / access": strong ? "The economic buyer, security leader, procurement owner, and technical evaluator are identified; an executive review is scheduled." : developing ? "The security champion is engaged and has offered to introduce the economic buyer, but direct buyer access is pending." : "The only contact is an individual contributor and no path to the economic buyer has been established.",
    "rating:Buying authority / access": rating(strong ? 0 : -1),
    "qualification:Budget and funding confidence": strong ? "Funding is allocated in the current fiscal year and the customer has confirmed the expected purchase range." : developing ? "A likely funding source has been identified, but final allocation and approval are still pending." : "No approved budget or confirmed funding source has been identified.",
    "rating:Budget and funding confidence": rating(strong ? 0 : -1),
    "qualification:Timing / compelling event": strong ? "The customer must complete implementation before its fiscal-year audit and contract-renewal deadline." : developing ? "The customer is targeting this fiscal year, but no immovable deadline has been confirmed." : "There is no documented deadline, mandate, renewal, audit, or other compelling event.",
    "rating:Timing / compelling event": rating(strong ? 0 : -1),
    "qualification:Solution fit and expected value": strong ? "The proposed platform directly addresses the documented workflow and is expected to reduce audit preparation and evidence collection substantially." : developing ? "The primary use case appears aligned, but technical requirements and expected value still require discovery." : "The customer has not confirmed required capabilities, measurable outcomes, or why this solution is preferable.",
    "rating:Solution fit and expected value": rating(),
    ...
    "rating:Engagement / activity": rating(strong ? 0 : -1),
    "qualification:Procurement path / feasibility": strong ? "The contract vehicle, procurement owner, legal review, security review, and target award process are documented." : developing ? "Procurement has been identified, but the contract vehicle and security-review timeline are still being mapped." : "No procurement contact, contract vehicle, approval sequence, or acquisition timeline is known.",
    "rating:Procurement path / feasibility": rating(strong ? 0 : -1),
    "qualification:Competitive position": strong ? "The customer has positioned the solution as the preferred option based on automation and public-sector workflow fit." : developing ? "Two alternatives are under review; differentiation is understood, but preference has not been established." : "The incumbent and competing alternatives are unknown, and no differentiation has been validated with the customer.",
    "rating:Competitive position": rating(),
    "qualification:Strategic value / expansion": strong ? "A successful initial deployment can expand across additional agencies, departments, and adjacent compliance workflows." : developing ? "There is a plausible second-department expansion path, but no executive expansion plan exists yet." : "The opportunity currently appears limited to a small single-use deployment with no confirmed expansion path.",
    "rating:Strategic value / expansion": rating(),
    "qualification:Existing relationships": strong ? "The account team has trusted relationships with the security sponsor, procurement lead, and an executive stakeholder." : developing ? "One prior relationship exists with the project champion; broader account relationships are still developing." : "There are no established customer relationships beyond the original lead contact.",
    "rating:Existing relationships": rating(strong ? 0 : -1),
    "qualification:Partner relationships": partner !== "—" ? `${partner} is actively engaged, has account access, and is supporting procurement and close planning.` : "No partner or reseller is currently attached; the opportunity is being pursued as a direct sale.",
    "rating:Partner relationships": partner !== "—" ? rating() : "1",
    ...
    "rating:Resource requirements": rating(strong ? 0 : developing ? -1 : 0)
  };
}
...
const defaultRatings = () => Object.fromEntries(categories.map(([label]) => [String(label), 1]));
...
function scoreFromRow(row: Record<string, string>, index: number): Opportunity {
  ...
  const aliases: Record<string, string[]> = {
    ...
    "Buying authority / access": ["Buying Authority", "Economic Buyer"], "Budget and funding confidence": ["Budget Status", "Budget", "Funding Confidence"],
    "Timing / compelling event": ["Compelling Event", "Timing"], "Solution fit and expected value": ["Solution Fit", "Expected Value"],
    "Engagement / activity": ["Engagement Activity", "Activity"], "Procurement path / feasibility": ["Procurement Path", "Procurement"],
    "Competitive position": ["Competition", "Competitive Position"], "Strategic value / expansion": ["Strategic Value", "Expansion Potential"],
    "Existing relationships": ["Existing Relationship", "Existing Relationships"], "Partner relationships": ["Partner Relationship", "Partner Relationships"],
...
export default function Dashboard({ userEmail }: { userEmail: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!data?.length) {
        ...
        setOpportunities(seed.map(item => ({ ...item, dbId: ids.get(item.id)?.id, lifecycle: "active", updatedAt: ids.get(item.id)?.updated_at })));
      } else {
        setOpportunities(data.map(row => ({ ...(row.source_data as Opportunity), dbId: row.id, lifecycle: row.lifecycle, updatedAt: row.updated_at, lastScoredAt: row.last_scored_at, history: (row.score_runs || []).sort((a: ScoreHistory, b: ScoreHistory) => b.created_at.localeCompare(a.created_at)) })));
      }
  ...
  const rescoreOpportunity = async (opportunity: Opportunity, categoryRatings: Record<string, number>, notes: string) => {
    ...
    try {
      const response = await fetch("/api/score", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ opportunity: { ...opportunity, manualCategoryRatings: categoryRatings, qualificationNotes: notes }, scoringModel: currentModel }) });
      if (response.ok) {
        const ai = await response.json(); const overall = Math.round(Number(ai.overallScore));
  ...
  const rescoreSelected = async () => {
    const targets = opportunities.filter(item => selected.includes(item.id));
    for (const item of targets) await rescoreOpportunity(item, Object.fromEntries(categories.map(([label]) => [String(label), Number(item.dealData?.[`rating:${String(label)}`]) || 1])), item.dealData?.qualificationNotes || "");
  ...
  const openActivity = async () => {
    ...
    setActivity((data || []) as ActivityLog[]); setShowActivity(true);
  };
  const saveModel = async (model: (string | number)[][]) => {
    ...
    for (const item of activeOpportunities) await rescoreOpportunity(item,Object.fromEntries(model.map(([label])=>[String(label),Number(item.dealData?.[`rating:${String(label)}`])||1])),item.dealData?.qualificationNotes||"");
    return true;
  };
  return <main>
    <header className="topbar"><div className="brand"><img className="brand-logo" src="/ilma-logo.png" alt="Ilma llama logo"/><span>Ilma&apos;s Route to Revenue</span></div><nav><a className={!showActivity?"active":""} onClick={()=>setShowActivity(false)}>Opportunities</a>{role==="admin"&&<a className={showActivity?"active":""} onClick={openActivity}>Admin activity</a>}<a onClick={() => setShowSettings(true)}>Settings</a></nav><div className="top-actions"><span className="sync"><span className="sync-dot" />{loadingData ? "Loading workspace…" : notice}</span><span className="user-email">{userEmail}{role==="admin"?" · Admin":""}</span><button className="icon-btn" onClick={logout} aria-label="Sign out"><LogOut size={16}/></button></div></header>
    {showActivity ? <AdminActivity logs={activity} query={activityQuery} setQuery={setActivityQuery} onRefresh={openActivity}/> : <>
...
function AdminActivity({ logs, query, setQuery, onRefresh }: { logs: ActivityLog[]; query: string; setQuery: (value:string)=>void; onRefresh: ()=>void }) {
...
function OpportunityDrilldown({ opportunity, onClose, onOverride, onRescore }: { opportunity: Opportunity; onClose: () => void; onOverride: (value: number, reason: string) => void; onRescore: (opportunity: Opportunity, ratings: Record<string, number>, notes: string) => void }) {
  ...
  const [ratings, setRatings] = useState<Record<string, number>>(() => Object.fromEntries(categories.map(([rawLabel]) => { const label = String(rawLabel); const stored = opportunity.analysis?.categoryScores[label]?.rating ?? Number(opportunity.dealData?.[`rating:${label}`]); return [label, Number.isFinite(stored) && stored >= 1 ? stored : 1]; })));
  const [notes, setNotes] = useState("");
  const analysisNames: Record<string, string> = {
    "Ideal customer fit": "Fit Analysis", "Pain and urgency": "Pain Analysis", "Buying authority / access": "Authority Analysis",
    "Budget and funding confidence": "Budget Analysis", "Timing / compelling event": "Timing Analysis", "Solution fit and expected value": "Solution Analysis",
    "Engagement / activity": "Activity Analysis", "Procurement path / feasibility": "Procurement Analysis", "Competitive position": "Competitive Analysis",
    "Strategic value / expansion": "Strategic Analysis", "Existing relationships": "Relationship Analysis", "Partner relationships": "Partner Analysis",
  ...
  const analysisText = (label: string, rating: number) => opportunity.analysis?.categoryScores[label]?.rationale || `The available evidence indicates a ${rating <= 2 ? "low" : rating === 3 ? "moderate" : "high"} level for this category and scores a ${rating} out of 5.`;
  return <tr className="drilldown-row"><td colSpan={11}><div className="drilldown">
    ...
    <section className="category-list"><div className="category-header"><div><h3>Full category detail</h3><p>Each tile shows the seller-entered value and the resulting category analysis.</p></div><button className="btn primary rescore" onClick={() => onRescore(editedOpportunity(), ratings, notes)}><Sparkles size={15}/> Save changes &amp; re-score</button></div>
      {categories.map(([rawLabel, weight]) => { const label = String(rawLabel); const rating = ratings[label]; return <article className="category-detail-tile" key={label}><div className="category-tile-head"><div><strong>{label}</strong><span>Weight {weight}%</span></div><div className="rating"><select aria-label={`${label} rating`} value={rating} onChange={e => setRatings(current => ({ ...current, [label]: Number(e.target.value) }))}>{[1, 2, 3, 4, 5].map(value => <option key={value} value={value}>{value} / 5</option>)}</select><i style={{ width: `${rating * 20}%` }} /></div></div><div className="category-tile-body"><label><b>Field Value:</b><textarea value={fieldValue(label)} placeholder={`Enter evidence for ${label.toLowerCase()}`} onChange={e=>setDealData(current=>({...current,[`qualification:${label}`]:e.target.value}))}/></label><p><b>{analysisNames[label] || "Category Analysis"}:</b> {analysisText(label, rating)}</p></div></article>; })}
      <label className="qualification-notes">Qualification notes<textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Add what you learned about budget, buyer access, timing, competition, procurement, partners, resources, or relationships…" /></label>
    </section>
    ...
    <section className="override"><button className="btn secondary" onClick={() => setEditing(!editing)}>Override overall score</button>{editing && <div className="override-form"><label>New score<input type="number" min="0" max="100" value={score} onChange={e => setScore(Number(e.target.value))}/></label><label>Reason <textarea value={reason} onChange={e => setReason(e.target.value)} placeholder="Required to preserve the scoring audit trail" /></label><button disabled={!reason.trim()} className="btn primary" onClick={() => { onOverride(score, reason); setEditing(false); }}>Save override</button></div>}</section>
  </div></td></tr>;
}
...
function SettingsPanel({ model, role, onClose, onSave }: { model: (string|number)[][]; role: string; onClose: () => void; onSave: (model:(string|number)[][])=>Promise<boolean> }) {
  ...
  return <div className="overlay"><section className="modal settings-modal"><button className="close" onClick={onClose}><X size={19}/></button><div className="eyebrow"><Settings2 size={15}/> Shared scoring model</div><h2>Qualification model</h2><p>Changes create a new version and automatically re-score active opportunities. Admin access is required.</p><div className="settings-list">{draft.map(([name, weight],index) => <div key={String(name)}><strong>{name}</strong><label><input value={String(weight)} min="0" max="100" type="number" disabled={role!=="admin"} onChange={e=>setDraft(current=>current.map((item,i)=>i===index?[item[0],Number(e.target.value)]:item))}/>%</label></div>)}</div><div className="settings-footer"><span className={total===100?"positive":"warning-text"}>Total {total}% · {role} access</span><button className="btn primary" disabled={role!=="admin"||total!==100||saving} onClick={async()=>{setSaving(true);if(await onSave(draft))onClose();setSaving(false)}}>{saving?"Saving…":"Save and re-score"}</button></div></section></div>;
}
...
function DealWorkspace({ onClose, onSave }: { onClose: () => void; onSave: (deal: Omit<Opportunity, "score" | "tier" | "changed" | "confidence">) => void }) {
  ...
  useEffect(() => {
    const timer = window.setTimeout(async () => {
      ...
      try {
        const manualCategoryRatings = Object.fromEntries(Object.entries(ratings).filter(([, value]) => value !== 1));
        const response = await fetch("/api/score", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ opportunity: { ...opportunity, salesforceFields: data, manualCategoryRatings, qualificationNotes: data.qualificationNotes || "" }, scoringModel: categories }) });
  ...
  const submit = () => {
    ...
    const effectiveRatings = Object.fromEntries(categories.map(([label]) => [String(label), analysis?.categoryScores[String(label)]?.rating ?? ratings[String(label)]]));
    onSave({ ...opportunity, dealData: { ...data, qualificationNotes: data.qualificationNotes || "", ...Object.fromEntries(Object.entries(effectiveRatings).map(([key, value]) => [`rating:${key}`, String(value)])) } });
  };
