...
const departmentNames = ["Department of Children and Families","Department of Health and Human Services","Office of Information Technology","Department of Public Safety","Department of Revenue","Department of Transportation","Office of the State Auditor","Department of Administration","Procurement Services Division","Office of Cybersecurity"];
...
const generatedSeed: Opportunity[] = Object.entries(generatedCounts).flatMap(([segment,count],segmentIndex)=>Array.from({length:count},(_,index)=>{
  ...
  const generatedIndex=sequence-featuredSeed.length-1; const account=`${customerNames[generatedIndex%customerNames.length]} ${departmentNames[Math.floor(generatedIndex/customerNames.length)%departmentNames.length]}`;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!data?.length) {
        ...
        const existing=data.map(row => normalizeOpportunity({ ...(latestSamples.get(row.salesforce_opportunity_id)||(row.source_data as Opportunity)), dbId: row.id, lifecycle: row.lifecycle, updatedAt: row.updated_at, lastScoredAt: row.last_scored_at, history: (row.score_runs || []).sort((a: ScoreHistory, b: ScoreHistory) => b.created_at.localeCompare(a.created_at)) }));
...
function OpportunityDrilldown({ opportunity, model, onClose, onRescore }: { opportunity: Opportunity; model:(string|number)[][]; onClose: () => void; onRescore: (opportunity: Opportunity, points: Record<string, number>, notes: string) => void }) {
