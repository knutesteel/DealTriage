...
function OpportunityDrilldown({ opportunity, onClose, onRescore }: { opportunity: Opportunity; onClose: () => void; onRescore: (opportunity: Opportunity, points: Record<string, number>, notes: string) => void }) {
  ...
  const overrides = () => Object.fromEntries(categories.flatMap(([rawLabel,rawMaximum])=>{const label=String(rawLabel);const maximum=Number(rawMaximum);const raw=manualPoints[label];return raw!==""?[[label,Math.min(maximum,Math.max(0,Math.round(Number(raw)||0)))]]:[];}));
...
function DealWorkspace({ onClose, onSave }: { onClose: () => void; onSave: (deal: Omit<Opportunity, "score" | "tier" | "changed" | "confidence">) => void }) {
  ...
  useEffect(() => {
    const timer = window.setTimeout(async () => {
      ...
      try {
        const manualCategoryPoints = Object.fromEntries(categories.flatMap(([label,maximum])=>{const raw=manualPoints[String(label)];return raw!==""?[[String(label),Math.min(Number(maximum),Math.max(0,Math.round(Number(raw)||0)))]]:[];}));
