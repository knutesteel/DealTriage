...
function AdminActivity({ logs, query, setQuery, onRefresh, model, onSave }: { logs: ActivityLog[]; query: string; setQuery: (value:string)=>void; onRefresh: ()=>void; model:(string|number)[][]; onSave:(model:(string|number)[][])=>Promise<boolean> }) {
  type ActivityColumn="when"|"user"|"action"|"area"|"record";
  const [sortColumn,setSortColumn]=useState<ActivityColumn>("when");
  ...
  const users=[...new Set(logs.map(log=>log.actor_email||"System"))].sort();
  const actions=[...new Set(logs.map(actionLabel))].sort();
  const areas=[...new Set(logs.map(areaLabel))].sort();
  const value=(log:ActivityLog,column:ActivityColumn)=>column==="when"?Date.parse(log.created_at):column==="user"?(log.actor_email||"System").toLowerCase():column==="action"?actionLabel(log).toLowerCase():column==="area"?areaLabel(log).toLowerCase():describe(log).toLowerCase();
  const visible=logs.filter(log=>{const when=Date.parse(log.created_at);const through=dateTo?Date.parse(`${dateTo}T23:59:59.999`):Infinity;return `${log.actor_email||"system"} ${actionLabel(log)} ${areaLabel(log)} ${describe(log)} ${log.entity_id||""}`.toLowerCase().includes(query.toLowerCase())&&(userFilter==="All"||(log.actor_email||"System")===userFilter)&&(actionFilter==="All"||actionLabel(log)===actionFilter)&&(areaFilter==="All"||areaLabel(log)===areaFilter)&&describe(log).toLowerCase().includes(recordFilter.toLowerCase())&&(!dateFrom||when>=Date.parse(`${dateFrom}T00:00:00`))&&when<=through;}).sort((a,b)=>{const av=value(a,sortColumn),bv=value(b,sortColumn);const compared=typeof av==="number"&&typeof bv==="number"?av-bv:String(av).localeCompare(String(bv));return sortDirection==="asc"?compared:-compared;});
  const toggleSort=(column:ActivityColumn)=>{if(sortColumn===column)setSortDirection(current=>current==="asc"?"desc":"asc");else{setSortColumn(column);setSortDirection(column==="when"?"desc":"asc");}};
  const Header=({column,children}:{column:ActivityColumn;children:ReactNode})=><button className="sort-header" onClick={()=>toggleSort(column)}>{children}<ChevronDown size={13} className={sortColumn===column?sortDirection:""}/></button>;
  const clearFilters=()=>{setQuery("");setUserFilter("All");setActionFilter("All");setAreaFilter("All");setRecordFilter("");setDateFrom("");setDateTo("");};
  return <section className="shell admin-activity"><div className="eyebrow"><Settings2 size={15}/> Administrator Only</div><div className="heading-row"><div><h1>Admin</h1><p>Manage the shared scoring model and review immutable workspace activity.</p></div><button className="btn secondary" onClick={onRefresh}>Refresh Activity</button></div><section className="admin-settings"><div className="section-title"><div><h2>Scoring Settings</h2><p>Add, delete, and allocate qualification categories. Expand each category for field expectations and scoring methodology.</p></div></div><ScoringModelEditor model={model} editable onSave={onSave}/></section><section className="metrics"><div className="metric"><span>Recorded Events</span><strong>{logs.length}</strong><small>Most recent 500 events</small></div><div className="metric"><span>Active Users</span><strong>{new Set(logs.map(log=>log.actor_email).filter(Boolean)).size}</strong><small>Authenticated actors in this log</small></div><div className="metric"><span>Score Events</span><strong>{logs.filter(log=>log.entity_type==="score_runs").length}</strong><small>Creates, edits, and re-scores</small></div></section><section className="table-card"><div className="activity-toolbar"><div className="table-title"><h2>All Activity</h2><span>{visible.length} matching events</span></div><div className="activity-filters"><label className="filter-control search-control">Search<span className="search"><Search size={16}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Any field"/></span></label><label className="filter-control">Date From<input type="date" value={dateFrom} onChange={e=>setDateFrom(e.target.value)}/></label><label className="filter-control">Date To<input type="date" value={dateTo} onChange={e=>setDateTo(e.target.value)}/></label><label className="filter-control">User<select value={userFilter} onChange={e=>setUserFilter(e.target.value)}><option>All</option>{users.map(item=><option key={item}>{item}</option>)}</select></label><label className="filter-control">Action<select value={actionFilter} onChange={e=>setActionFilter(e.target.value)}><option>All</option>{actions.map(item=><option key={item}>{item}</option>)}</select></label><label className="filter-control">Area<select value={areaFilter} onChange={e=>setAreaFilter(e.target.value)}><option>All</option>{areas.map(item=><option key={item}>{item}</option>)}</select></label><label className="filter-control">Record<input value={recordFilter} onChange={e=>setRecordFilter(e.target.value)} placeholder="Record name or ID"/></label><button className="btn secondary clear-activity" onClick={clearFilters}>Clear</button></div></div><div className="table-wrap"><table><thead><tr><th><Header column="when">When</Header></th><th><Header column="user">User</Header></th><th><Header column="action">Action</Header></th><th><Header column="area">Area</Header></th><th><Header column="record">Record</Header></th></tr></thead><tbody>{visible.map(log=><tr key={log.id}><td><strong>{new Date(log.created_at).toLocaleDateString()}</strong><span>{new Date(log.created_at).toLocaleTimeString()}</span></td><td>{log.actor_email||"System"}</td><td><span className="stage">{actionLabel(log)}</span></td><td>{areaLabel(log)}</td><td>{describe(log)}</td></tr>)}</tbody></table></div></section></section>;
}
